...
contract ZkVerifyRiskSurface {
    ...
    /// @dev sha256("ultrahonk:v0.84"). The pallet folds a version hash into the
    ///      statement between the vk hash and the public inputs, so it is pinned
    ///      here and the whole formula is tested against a statement zkVerify
    ///      returned rather than against a reimplementation of it.
    ...
    /// @dev    Take this from zkVerify's own RPC, `session.getVkHash`. It hashes
    ///         the SCALE-encoded versioned key rather than the raw bytes, so the
    ///         two differ. Binding it at deployment is what stops a valid proof
    ///         of a different statement being presented here.
    ...
    ///
    ///         Checked against a statement zkVerify returned for a real
    ///         submission, which is the only way to know the encoding is right.
